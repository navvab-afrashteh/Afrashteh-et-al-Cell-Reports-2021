...
function R1_Artur_fig_bar_graph_amplitude_riseTime_1_glut_LoMedCombined(ss)
clc; close all;
if ~exist('ss','var')
    ss = 1;
end
...
%     end
% end
% save('Y:\homes\navvab.afrashteh\CloudStation\Navvab-Sam\NeuroPhotonic Paper\Test\Figures_new_code_AC\ac_glut_motifs_averaged_data.mat','valsE','valsSCC','valsS');
load('Y:\homes\navvab.afrashteh\CloudStation\Navvab-Sam\NeuroPhotonic Paper\Test\Figures_new_code_AC\ac_glut_motifs_averaged_data.mat');
...
annovaVar = [avgValsS' avgValsSCC'];% avgValsE(:,1) avgValsE(:,2) avgValsE(:,3)];
disp('RiseTime')
% prT = R1_ranovaGlut(annovaVar);
% thisCols = {cols.eSponColors{1},cols.sponColorsN{1},cols.stimLevelColors{1},cols.stimLevelColors{3},cols.stimColors{4}};
thisCols = {'b',cols.sponColorsN{1},cols.stimLevelColors{1},cols.stimLevelColors{3},cols.stimColors{4}};
...
set(gca,'XTick',1:size(annovaVar,2),'YTick',[0:50:150]);
XTickLabels = {'ex PCC', 'PCC'};
set(ff.ha,'XTickLabel',{'ex PCC', 'PCC'});
xtickangle(ff.ha,0);
...
ff = makeFigureWindow__one_axes_only(100,[4 1 2.5 2],[0.15 0.2 0.82 0.75]);
thisCols = {'b',cols.sponColorsN{1},cols.stimLevelColors{1},cols.stimLevelColors{2},cols.stimLevelColors{3},cols.stimColors{4}};
hold on;
avgAvgPlotSCC = mean(allAvgPlotsSCC);
plot(x_axisS,avgAvgPlotSCC,'color',thisCols{2});
semAvgPlotSCC = std(allAvgPlotsSCC)/sqrt(length(allAvgPlotsSCC));
shadedErrorBar(x_axisS,avgAvgPlotSCC,semAvgPlotSCC,{'color',thisCols{2}},0.75);
avgAvgPlotS = mean(allAvgPlotsS);
plot(x_axisS,avgAvgPlotS,'color',thisCols{1});
semAvgPlotS = std(allAvgPlotsS)/sqrt(length(allAvgPlotsS));
shadedErrorBar(x_axisS,avgAvgPlotS,semAvgPlotS,{'color',thisCols{1}},0.75);
...
pos = get(hy,'Position');pos(1) = pos(1) + 2;set(hy,'Position',pos);
x1 = 250; x2 = x1+25; y1 = (0.5:0.05:7); y1 = y1(1:4); y2 = y1;
legendFontSize = 6-1;
legs = {'ex PCC','PCC'};
iii = 0;
