function Artur_Trajectory_VSDI_VC
...
if strcmp(strtrim(name),'mohajem3-psy13')
    pathPrefix = 'F:\CloudStation\Navvab-Sam\NeuroPhotonic Paper\Test\';
end
pathPrefix = 'Y:\homes\navvab.afrashteh\CloudStation\Navvab-Sam\NeuroPhotonic Paper\Test\';
...
return;
    ...
    function FindSaveTraj(ss)
        ...
        for an = 1:length(animalNumber)
            ...
            % Traj for Evoked
            [lists, names]= getEvokedListsDiffStimAmps_artur(animalNumber(an),0);
    ...
    function plotTraj(ss)
        nRows = 2; nCols = 5; sprc = [10 10]; whUnits = [-15 -15]; rtup = [30 0]; rbUnits = [0 0];
        ...
        for an = 1:length(animalNumber)
            ...
            LMHnames = {'Lo', 'Med', 'Hi'};
            for LMH = 1%:3
                ax1 = axes('parent',hf, 'Position',paPositionBoth(2,:));hold on;
                [dim1, dim2] = size(mask);
                imE = zeros(dim1, dim2);
                Strials = 25;%size(Straj{an}.TrajFrames,1);
                Etrials = length(Etraj{an}.Trials{1});
                perm = sort(randperm(Etrials,Strials));
                eX = Etraj{an}.Xinterp{selLMH(LMH)}(perm,:);
                eY = Etraj{an}.Yinterp{selLMH(LMH)}(perm,:);
                for jj = 1:size(eX,1)
                    I = zeros(dim1, dim2);
                    xi = eX(jj,:);
                    yi = eY(jj,:);
                ...
                if an == 1
                    ...
                    ht = text(xt,yt,txtStr,'HorizontalAlignment','center','VerticalAlignment','middle','rotation',90,'color',cols.avgStimColors{1});
                end
            ...
            imS = zeros(dim1, dim2);
            sX = Straj{an}.Xinterp(1:Strials,:);
            sY = Straj{an}.Yinterp(1:Strials,:);
            for jj = 1:size(sX,1)
                I = zeros(dim1, dim2);
                xi = sX(jj,:);
                yi = sY(jj,:);
            ...
            if an == 1
                xt = -5; yt = 64.5;
                txtStr = 'Spon';
                text(xt,yt,txtStr,'HorizontalAlignment','center','VerticalAlignment','middle','rotation',90,'color',cols.sponColors{1})
                xt = -10; yt = -5;
        ...
        plotBarGraphFDs (ff,animals,FD,meta.colors,meta)
    ...
    function plotBarGraphFDs (ff,animals,FD,cols,meta)
        ...
        thisCols = {cols.sponColorsN{1},cols.stimLevelColors{3},cols.stimLevelColors{2},cols.stimLevelColors{3},cols.stimColors{4}};
        ...
        pos = get(hy,'Position');pos(1) = pos(1)+0.25;set(hy,'Position',pos);
        ...
        set(ff.ha,'XTickLabel',meta.stimNamesVC);
        xtickangle(ff.ha,45);
