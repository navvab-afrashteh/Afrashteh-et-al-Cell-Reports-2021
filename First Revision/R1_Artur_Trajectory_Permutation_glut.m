function R1_Artur_Trajectory_Permutation_glut
clc; close all;
...
return;
    ...
    function runPerm(ss,ppType)
        ...
        if HPF
            FDPermFileName = sprintf('%s\\Fractional Analysis_Permutation_%s_glut_HPF.mat',pathPrefix ,match{ss});
        else
            FDPermFileName = sprintf('%s\\Fractional Analysis_Permutation_%s_glut_BPF_050Hz_6Hz.mat',pathPrefix ,match{ss});
        end
...
%             disp(['animal #',num2str(an),': K-S test: h = ', num2str(hSR(an)),', p = ', num2str(pSR(an))])
    ...
    function pfileName = deletePDF (pdfn,pfn)
        if pdfFlag
            pfileName = sprintf('R1_Artur_Fractal Dimension PDF_%s_glut_%d.pdf',match{pfn},pdfn);
