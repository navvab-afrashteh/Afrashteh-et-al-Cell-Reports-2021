function Artur_figure_1_Distributions_2_glut(ss)
close all
clc
...
faceAlpha = 0.75;
for ii = 1:length(selLMH)
    axes(ff.ha); shadedErrorBar(cInterp,cEmean{ii},cEsem{ii},{'color',cols.stimLevelColors{selLMH(ii)}/1.1},faceAlpha);
end
...
save2pdf(makeName(sprintf('fig_distributions_glut_%d.pdf',ss),getpdffolder),ff.hf,600);
