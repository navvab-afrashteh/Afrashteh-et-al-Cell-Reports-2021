function R1_EVokSponDuration_VSD(ss)
clc; close all;
if ~exist('ss','var')
    ss = 1;
end
stim = {'FL' 'HL' 'VC'};
pdfFileName = sprintf('R1_Evok_High_Spon_Duration_%s.pdf',stim{ss});
...
%{
if VCstim
    fileName = makeName('eDataVC.mat',mainDataFolder);
    eData = load(fileName); mdataE = eData.mdata;
    fileName = makeName('sDataDFthVC.mat',mainDataFolder);
    sDataDFth = load(fileName); mdataSDFth = sDataDFth.mdata;
    d = mdataSDFth{1};
else
    fileName = makeName('eData.mat',mainDataFolder);
    eData = load(fileName); mdataE = eData.mdata;
...
ninterp = nbins*2;
minX = 0;
maxX = (xM+1); maxX = maxX*1000/150;% frames to msec
...
for an = 1:length(animalNumber)
    ...
    [~, names]= getEvokedListsDiffStimAmps(animalNumber(an),multLevels);
    listR = listOfRecordingsSpon(animalNumber(an));
    if multLevels
        selLMH = 3;
        LMHname = names{ss}{mdataE{ss}.anGroup{an}{selLMH}};
    else
        LMHname = names{ss}{1};
    end
    ...
    bVarE = Edt; bVarE = bVarE*1000/150;
    bVarSCC = Sdt; bVarSCC = bVarSCC*1000/150;
...
xticks = 0:max(cInterp)/3:max(cInterp); xticks = 10*round(0.1*xticks);
