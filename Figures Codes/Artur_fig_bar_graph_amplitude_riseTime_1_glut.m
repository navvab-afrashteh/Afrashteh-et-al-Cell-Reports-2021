function Artur_fig_bar_graph_amplitude_riseTime_1_glut(ss)
close all
if ~exist('ss','var')
    ss = 1;
end
...
%         avgValsE(an,iii) = mean(valsE{an}.alldf{ii});
        avgValsE(an,iii) = valsE{an}.avg.Amp(ii);
    end
...
save2pdf(makeName('allCurves_df_glut.pdf',getpdffolder),hf,600);
...
x1 = 240; x2 = x1+40; y1 = (0.79:0.07:7); y1 = y1(1:4); y2 = y1;
legendFontSize = 6-1;
% legs = {'Spont-CC','Lo Evok','Med Evok','Hi Evok'};
legs = meta.stimNames;
for ii = 1:4
