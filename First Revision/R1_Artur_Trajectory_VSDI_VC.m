function R1_Artur_Trajectory_VSDI_VC
clc;close all;
...
return;
    ...
    function plotTraj(ss)
        ...
        rng(1);
        FD_E = zeros(length(animalNumber),1);
        FD_S = zeros(length(animalNumber),1);
        % Create axes
        for an = 1:length(animalNumber)
        ...
        % calculated in R1_Artur_Trajectory_Permutation_VSDI_VC.m file
        FDPermFileName = sprintf('%s\\Fractional Analysis_Permutation_%s.mat',pathPrefix ,match{ss});
        load(FDPermFileName,'FD_E','FD_S')
        FD_E = mean(FD_E,2); FD.FD_E = FD_E;
        FD_S = mean(FD_S,2); FD.FD_S = FD_S;
        [prT,hrT] = ranksum(FD_S,FD_E,'Tail','right')
        pdfn = 2*[101 101 101];
