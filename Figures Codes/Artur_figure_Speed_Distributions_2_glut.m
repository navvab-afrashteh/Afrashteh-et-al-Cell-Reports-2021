function Artur_figure_Speed_Distributions_2_glut (ss,roi)
...
for an = 1:length(animals)
    ...
    for iii = 1:length(selLMH)
        ...
        % bVarEA{iii} = valsE{ann}.absalluv{ii}; % Original
        % bVarEA{iii} = max(mean(valsE{ann}.allThisuv{ii}(:,16:end))) - min(mean(valsE{ann}.allThisuv{ii}));
        bVarEA{iii} = max(valsE{an}.allThisuv{ii}(:,16:end).')-min(valsE{an}.allThisuv{ii}.');
        bVarEC = [bVarEC bVarEA{iii}];
        avgValsE(an,iii) = mean(bVarEA{iii});
    end
    ...
    % bVarSCC = abs(valsSCC{ann,selSCC}.alluv(inds))*6.667;
    % bVarSCC = max(mean(valsSCC{ann,selSCC}.allThisuv(inds,:))) - min(mean(valsSCC{ann,selSCC}.allThisuv(inds,:)));
    bVarSCC = max(valsSCC{ann,selSCC}.allThisuv(inds,:).')-min(valsSCC{ann,selSCC}.allThisuv(inds,:).');
    % bVarSCC = valsSCC{ann,selSCC}.absalluv; % Original
...
for ii = 1:length(selLMH)
    axes(ff.ha);
    shadedErrorBar(cInterp,cEmean{ii},cEsem{ii},{'color',cols.stimLevelColors{selLMH(ii)}/1.1},faceAlpha);
end
...
xlabel('Average Speed (mm/sec)','FontSize',ylabelFontSize);
xlim([minX maxX]);
ylim([minY maxY]);
axes(ff.ha);
...
x1 = 15; x2 = x1+2; y1 = (0.1:0.06:0.7); y1 = y1(1:4); y2 = y1;
...
save2pdf(makeName(sprintf('fig_distributions_speed_glut_ss_%d_roi_%d.pdf',ss,roi),getpdffolder),ff.hf,600);
nnn=2;
