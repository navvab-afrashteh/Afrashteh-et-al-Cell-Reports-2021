function R1_Artur_Trajectory_Permutation_VSDI_VC
clc; close all;
...
if plotPermFlag
    pdfFlag = 1;
    fineSp = 0.001;
    legendFontSize = 8; xlabelFontSize = 9; ylabelFontSize = 9;
    titleFontSize = 8;
    ss = 3; % VC
    plotPerm(ss);
end
...
return;
    ...
    function runPerm(ss)
        ...
        for ann = 1:length(animals)
            ...
            LMH = 1;
            [dim1, dim2] = size(mask);
            allXe = Etraj{an}.Xinterp{selLMH(LMH)};
            allYe = Etraj{an}.Yinterp{selLMH(LMH)};
            ne = size(allXe,1);
            p = randperm(ne);
            allXe = allXe(p,:);
            allYe = allYe(p,:);
    ...
    function plotPerm(ss)
        close all
        nRows = 1; nCols = 4+1; sprc = [10 30]; whUnits = [-34 -300]; 
