function HS_on_file_HPF(filePath,fileName, varName, masks)
[r, c] = find(masks.mask);
...
%     [im1, im2] = normalize_two_consequtive_frames(im1,im2,idxMask);
    [u, v] = HS(im1,im2,HSparams_default.alpha,HSparams_default.iterations);
%     [u, v, ~] = Coarse2FineTwoFrames(im1,im2,CLG_parameters);
    uv(:,:,idx) = (u +1i*v);
end
display('Done !!!');
% close(hWaitBar);
uvFile = makeName(sprintf('uvHS_%s',fileName),filePath);
