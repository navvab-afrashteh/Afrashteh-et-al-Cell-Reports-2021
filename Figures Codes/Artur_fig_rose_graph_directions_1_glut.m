function Artur_fig_rose_graph_directions_1_glut(ss,roi)
...
ff = makeFigureRowsCols(111,[6 2 4.5135 1.75],'RowsCols',[1 3],'spaceRowsCols',[0.1 0.0205],'rightUpShifts',[0.0782 0.01],'widthHeightAdjustment',[-40 -80]);
% ff = makeFigureRowsCols(112,[6 2 5.9 1.75],'RowsCols',[1 4],'spaceRowsCols',[0.1 0.03],'rightUpShifts',[0.06 0.01],'widthHeightAdjustment',[-40 -80]);
sFactors = [0.85 0.8 0.85 0.8];
...
for an = length(animals):-1:1
    for iii = 1:length(LH)
        ...
        if animals(an) == 44
            if ii == 1
                theAng = angle(mean([valsE{an}.allang{ii},valsE{an}.allang{ii+1}]))+pi;
                theMag = abs(mean([valsE{an}.allang{ii},valsE{an}.allang{ii+1}]));
            else
                theAng = angle(mean(valsE{an}.allang{ii}))+pi;
                theMag = abs(mean(valsE{an}.allang{ii}));
            end
            avgValsE(an,iii) = conj(theMag * exp(1i * theAng));
        else
            if ii == 1
                avgValsE(an,iii) = conj(mean([valsE{an}.allang{ii},valsE{an}.allang{ii+1}]));
            else
    ...
    if animals(an) == 44
        theAng = angle(mean(valsSCC{an,selSCC}.allang))+pi; 
        theMag = abs(mean(valsSCC{an,selSCC}.allang));
        avgValsSCC(an) = conj(theMag * exp(1i * theAng));
        theAng = angle(mean(valsS{an}.allang))+pi; 
        theMag = abs(mean(valsS{an}.allang));
...
% [~,~,stats] = anova1(annovaVar,{'SpontCC','Evok_1','Evok_2'},'off');
...
set(ff.ha,'XTickLabel',meta.stimNames([1,2,4]),'FontSize',axesFontSize);
xtickangle(ff.ha,45);
...
for an = 1:length(animals)
    ...
    if selLMH == 1
        angE = [angE,angle(conj(valsE{an}.allang{selLMH+1}))];
    end
    if an == 4
        angE = angle(conj(-valsE{an}.allang{selLMH}));
        if selLMH == 1
