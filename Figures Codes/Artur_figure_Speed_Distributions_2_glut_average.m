function Artur_figure_Speed_Distributions_2_glut_average (ss,roi)
close all
clc
...
for an = 1:length(animals)
    ...
    for iii = 1:length(selLMH)
        ...
        bVarEA{iii} = abs(valsE{ann}.avg.Speed(ii));
...
save2pdf(makeName(sprintf('fig_distributions_speed_glut_average_ss_%d_roi_%d.pdf',ss,roi),getpdffolder),ff.hf,600);
