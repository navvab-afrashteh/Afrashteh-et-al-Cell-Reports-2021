function Artur_fig_bar_graph_speed_directions_2_glut_avg_LoMedCombined(ss,roi)
clc
close all
...
for an = 1:length(animals)
    for ii = [1,3]%1:length(selLMH_all{ss}(an,:))
        if ii == 1
            avgValsE(an,ii) = mean([abs(valsE{an}.avg.Speed(ii)),...
                                    abs(valsE{an}.avg.Speed(ii+1))]);
        else
            avgValsE(an,ii) = abs(valsE{an}.avg.Speed(ii));
        end        
...
annovaVar = annovaVar./repmat(mean(annovaVar,2),1,3);
disp('Speed')
...
set(ff.ha,'XTickLabel',meta.stimNames([1,2,4]),'FontSize',axesFontSize-2);
xtickangle(ff.ha,45);
    ...
    for aaa = [1,3]%1:length(valsE{an}.allThisdf)
        if ~isempty(valsE{an}.AvgallThisdf{1}{aaa})
            ...
            if aaa == 1
                ...
                thisPlot = [thisPlot; valsE{an}.allThisuv{aaa+1}(inds,:)];
            end
            if ~isempty(thisPlot)
                if aaa == 1
                ...
                if aaa ==1
                    avgPlot = mean([valsE{an}.avg.uv_chopped{aaa},...
                                    valsE{an}.avg.uv_chopped{aaa+1}],2)';
                else
...
save2pdf(makeName('allCurves_uv_glut_LoMedCombined.pdf',getpdffolder),gcf,600);
...
x1 = 150; x2 = x1+40; y1 = linspace(2,3.5,4); y1 = y1(1:4); y2 = y1;
legendFontSize = 6-1;
% legs = {'Spont-CC','Lo Evok','Med Evok','Hi Evok'};
