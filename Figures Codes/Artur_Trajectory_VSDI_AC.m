function Artur_Trajectory_VSDI_AC
...
return;
    ...
    function FindSaveTraj(ss)
        ...
        for an = 1:length(animals)
            ...
            anPathName = makeName(animalNumber(an),getMainDataFolderAC);
            sPathName = makeName('pSpon',anPathName);
            ePathName = makeName('pEvoked',anPathName);
            
            % Traj for Evoked
            [lists, names] = getEvokedListsDiffStimAmps_glut(animalNumber(an),1,dataType);
            for level = 1:length(names{4})
                onsets = valsE{an}.onsets(level,:);
                pfs = valsE{an}.pfs(level,:);
                trials = getEvokedListsDiffStimAmpsTrials_glut(animalNumber(an),names{4}{level},dataType);
                ...
                % save results
                EtrajName = 'Etraj_';
                saveEtraj_FileName = sprintf('%sFigures_new_code\\%s%s_anaesthetics_allAn_04corr.mat',pathPrefix ,EtrajName,match{ss});
                if exist(saveEtraj_FileName,'file')
            ...
            disp(['evoked an = ' num2str(an), ' done!'])
            ...
            StrajName = 'Straj_';
            saveStraj_FileName = sprintf('%sFigures_new_code\\%s%s_anaesthetics_allAn_04corr.mat',pathPrefix ,StrajName,match{ss});
            if exist(saveStraj_FileName,'file')
            ...
            fileName =  ['ImgSeq_avg_an', num2str(an),'_' ,match{ss}, '_anaesthetics_all_04corr.raw'];
            imwriteallraw([pathPrefix '\' fileName],ImgSeq,'*float32')
            
            fileName =  ['ImgSeq_avg_an', num2str(an),'_' ,match{ss}, '_anaesthetics_all_04corr.mat'];
            save([pathPrefix '\' fileName],'ImgSeq','-v7.3')
            
            mask1.bigMask = getMask(animalNumber(an),'ec',1.1,'glut',0,'ac',1);
            mask1.mask = getMask(animalNumber(an),'glut',0,'ac',1);
            CLG_on_file(pathPrefix,fileName, 'ImgSeq', mask1)
            
            CorrMaxDF_frames_fileName = [pathPrefix '\CorrMaxDF_frames','_', match{ss}, '_anaesthetics_all_04corr.mat'];
            save(CorrMaxDF_frames_fileName,'CorrMaxDF_frames')
            
            disp(['spon an = ' num2str(an), ' done!'])
        end
    end

    function plotTraj(ss)
        nRows = 4; nCols = 7; sprc = [10 10]; whUnits = [-15 -15]; rtup = [30 0]; rbUnits = [0 0];
        ...
        FD.FD_S = FD_S;
        %         save(sprintf('Fractional Analysis_anaesthetics_%s.mat',match{ss}),'FD')
        %         load(sprintf('Fractional Analysis_anaesthetics_%s.mat',match{ss}),'FD')
        ...
        %         makeTempPDF;
        %         append_pdfs(pfileName,'temp.pdf');
        %         finalizepdf(pfileName);
...
% Tracking front-end of wave. Direction based on how contour stretch between two consecutive frames
    ...
    function pfileName = deletePDF (pdfn,pfn)
        if pdfFlag
            pfileName = sprintf('Artur_Traj_%s_%d.pdf',match{pfn},pdfn);
