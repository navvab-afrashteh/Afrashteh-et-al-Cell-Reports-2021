function R1_SponMeasRelation(ss,roi,selE)
close all
clc

match = {'FL' 'HL'};
if ~exist('ss','var')
    ss = 1; roi = 4; selE = [1,2,3];
end
...
% mfilename
fileName = sprintf('%s_ss%d_roi%d.mat',mfilename,ss,roi);
rewrite = 0;
if rewrite
...
fprintf('spon speed: m = %0.3f; pval = %0.10f; R2 = %0.2f\n',mS,pS,RsquaredS)
...
hy = ylabel('log Speed (mm/sec)','FontSize',ylabelFontSize);
...
x1 = 0.05; x2 = x1+0.1; y1 = (1.6:0.3:7); y1 = y1(1:4); y2 = y1;
legs = {'spon','evok'};
thisCols = {'b','r'};
iii = 0;
for ii = 1:length(legs)
    ...
    text(x2+0.02,y1(iii),legs{ii},'Color',thisCols{ii},'FontSize',8);
end

% CDFs
hcdf = axes('Position',[0.65,0.7,0.2,0.2]);
hold on
plot(ci,cdfE,'color','r','linewidth',1)
plot(ci,cdfS,'color','b','linewidth',1)
xM = round(max(ci));
xlabel('log(Speed)','FontSize',ylabelFontSize);
...
% display slopes and pvaleus
pS = median(pS); pE = median(pE);
fprintf('spon dir stability: m = %0.3f; pval = %0.10f; R2 = %0.2f\n',mS,pS,RsquaredS)
fprintf('evok dir stability: m = %0.3f; pval = %0.15f; R2 = %0.2f\n\n',mE,pE,RsquaredE)
...
plot(xS,yS,'color','b','linewidth',2)
plot(xE,yE,'color','r','linewidth',2)
yM = 1; ym = -1; xM = 1.5; if ss==2; xM = 1.2; end
% significancy
[~,sig] = isSignificant(pS);
xt = xM*0.5;
a = 0.2; if ss==2; a = 0.2; end
...
plot(xS,yS,'color','b','linewidth',2)
plot(xE,yE,'color','r','linewidth',2)
yM = 1.6; ym = 0.8; xM = 0.85; if ss==2; xM = 0.65; end
% significancy
[~,sig] = isSignificant(pS);
xt = xM*0.5;
a = 0.2; if ss==2; a = 0.2; end
