function Artur_Trajectory_Permutation_glut
clc; close all;
...
return;
    ...
    function runPerm(ss,ppType)
        ...
        if HPF
            FDPermFileName = sprintf('%sFigures_new_code_AC\\Fractional Analysis_Permutation_%s_glut_HPF.mat',pathPrefix ,match{ss});
        else
            FDPermFileName = sprintf('%sFigures_new_code_AC\\Fractional Analysis_Permutation_%s_glut_BPF_050Hz_6Hz.mat',pathPrefix ,match{ss});
        end
        save(FDPermFileName,'FD_E','FD_S')
    end

    function plotPerm(ss,ppType)
        % suppress warnings
        [msg, id] = lastwarn;
        warning('off', id)
        close all
...
%             disp(['animal #',num2str(an),': K-S test: h = ', num2str(hSR(an)),', p = ', num2str(pSR(an))])
    ...
    function pfileName = deletePDF (pdfn,pfn)
        if pdfFlag
            pfileName = sprintf('Artur_Fractal Dimension PDF_%s_glut_%d.pdf',match{pfn},pdfn);
