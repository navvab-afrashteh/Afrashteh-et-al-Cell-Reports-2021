function Artur_fig_bar_graph_amplitude_riseTime_1_AC_average(ss)
if ~exist('ss','var')
    ss = 1;
end
...
cols = getColors;
selSCC = 3;
amplitude_threshold = 1;%meta.amplitude_threshold;
% mfilename
...
% end
% save('ac_motifs_averaged_data.mat','valsE','valsSCC','valsS');
...
for ann = 1:length(animals)
    ...
    for iii = 1:3
        ii = iii;
        avgValsE(ann,iii) = abs(valsE{an}.avg.Amp(ii));
%         avgValsE(ann,iii) = abs(valsE{an}.Avgalldf{ii});
    end
    avgValsSCC(ann) = valsSCC{an,selSCC}.avg.Amp;
...
%                 avgPlot = mean(thisPlot);
%                 avgPlot = valsE{an}.AvgallThisdf{1}{aaa};
...
avgAvgPlotSCC = avgAvgPlotSCC-min(avgAvgPlotSCC(x_axis<0));%%%%%%
...
for aaa = 1:3
    ...
    for ann = 1:length(animals)
        an = animals(ann);
        allAvgPlotsE_LMH(ann,:) = allAvgPlotsE{an,selLMH_all{an}{aaa}};
    end
    avgAvgPlotE{aaa} = mean(allAvgPlotsE_LMH);
    avgAvgPlotE{aaa} = avgAvgPlotE{aaa} - min(avgAvgPlotE{aaa}((x_axis<0)));%%%%%%%%%
