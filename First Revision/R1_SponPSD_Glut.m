...
for an = length(animalNumber):-1:1
    ...
    PSD(an,:) = nanmean(10*log10(Pxx),1); % mean power in dB
end
...
colorSCC = 'k';
N = size(PSD,1);
cSCCmean = nanmean(PSD,1);
cSCCsem = nanstd(PSD,[],1)/sqrt(N);
...
xticks = [0.2,0.75,3,12,50];
set(gca,'xtick',xticks)
yticks = fliplr(0:-20:-100);
